...
- Where this specification requires an `Invalid(...)`, it MUST use the corresponding code defined by §14 (Behavior Diagnostics).
...
Error behavior:
- If `list` is not a List: `Invalid(...)` with code `ReduceElementsRight::NEED_LIST`.
...
Error behavior:
- If `list` is not a List: `Invalid(...)` with code `FoldElements::NEED_LIST`.
...
Error behavior:
- If elements are not comparable: `Invalid(...)` with code `SortElementsDescending::NEED_COMPARABLE_ELEMENTS`.
...
Error behavior:
- If keys are not comparable: `Invalid(...)` with code `SortElementsByDescending::NEED_COMPARABLE_KEYS`.
...
Error behavior:
- If any mapping result is not a List: `Invalid(...)` with code `FlatMapElements::NEED_LIST_RESULTS`.
...
Error behavior:
- If any element is not numeric: `Invalid(...)` with code `ProductElements::NEED_NUMERIC_ELEMENTS`.
...
Error behavior:
- If any element is not numeric: `Invalid(...)` with code `AverageElements::NEED_NUMERIC_ELEMENTS`.
- If list is empty: `Invalid(...)` with code `AverageElements::NEED_NONEMPTY_LIST`.
...
Error behavior:
- If lists have different lengths: `Invalid(...)` with code `MapFromLists::LENGTH_MISMATCH`.
- If duplicate keys: `Invalid(...)` with code `MapFromLists::DUPLICATE_KEY`.
...
Error behavior:
- If any key is not Text: `Invalid(...)` with code `RecordFromMap::NEED_TEXT_KEYS`.
...
This table consolidates all diagnostic codes defined in the Core Safe Transforms operator family.
...
**End of Behavior Vocabulary — Core Safe Transforms v1.0.0**
