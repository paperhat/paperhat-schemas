...
| Canonical       | JavaScript                   | Python | Java   | C#               | C / C++            | Rust   | Go                          | Swift  | Kotlin                    |
| --------------- | ---------------------------- | ------ | ------ | ---------------- | ------------------ | ------ | --------------------------- | ------ | ------------------------- |
| IsFinite(x)     | Direct: `Number.isFinite(x)` | Direct | Direct | Direct (.NET 5+) | Direct: `isfinite` | Direct | Emulate: `!IsInf`           | Direct | Direct                    |
...
| Target     | Mapping |
| ---------- | ------- |
| JavaScript | **Emulate** using sign-bit detection (must preserve negative zero): `CopySign(m,s) = AbsoluteValue(m) * (IsNegativeSignBit(s) ? -1 : 1)` where `IsNegativeSignBit(s)` is `s < 0 \|\| Object.is(s, -0)`. |
...
**End of Canonical Math API Surface — Backend Mapping Specification v1.0.0**
