...
This appendix is the canonical machine-readable source for these vectors in v1.0.0.

### Ontology and reasoning (Informative)

If you want conformance results to support practical querying and reasoning, model test intent, applicability, backend capability, mapping claims, and execution outcomes as Concepts + Traits that compile into triples. A companion conformance suite ontology specification is required to formalize this model.

### Authority and assertions (Normative)

The authority boundary for suites (suite vs prose) and the required "expected outcome" assertion forms MUST be defined by a companion conformance suite authority specification. That specification MUST define which assertion forms are required, how expected outcomes are encoded, and the scope boundary between suite-level and prose-level conformance claims.

**End of Canonical Math API Surface — Conformance Appendix v1.0.0**
