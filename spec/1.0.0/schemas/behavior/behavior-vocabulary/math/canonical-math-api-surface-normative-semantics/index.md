...
Returns `true` iff `x` is not ±infinity.

### 2.2 IsInfinite(x)

Returns `true` iff `x` is +infinity or −infinity.

### 2.3 IsPositiveInfinity(x), IsNegativeInfinity(x)
...
Returns $|x|$ with IEEE semantics for infinities. `AbsoluteValue(±Infinity)` returns `PositiveInfinity`.
...
For all functions below, if `divisor` is zero and `dividend` is nonzero, return `±Infinity` (sign follows standard rules). If both are zero, return `Invalid(...)` with code `Divide::INDETERMINATE_FORM`. This applies uniformly to all numeric types.
...
**End of Canonical Math API Surface — Normative Semantics v1.0.0**
