...
This specification defines v1.0.0 operators for working with the canonical missing-value concept `<Absent/>`.
...
v1.0.0 defines a single canonical missing-value concept:

- `<Absent/>`

v1.0.0 MUST NOT introduce any semantic `Null` value.
...
- Operators that require presence MUST reject `<Absent/>` with `Invalid(...)` rather than treating it as a domain value.
...
**End of Behavior Vocabulary — Presence and Missingness v1.0.0**
