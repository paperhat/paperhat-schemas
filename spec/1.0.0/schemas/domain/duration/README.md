...
| unit | $EnumeratedToken | yes | Temporal unit, e.g. `$seconds`, `$minutes`, `$hours`, `$days`, `$weeks`, `$months`, `$years`. Consuming schemas may constrain the allowed units. |
...
- Recipe's PreparationTime and CookingTime concepts duplicate this pattern with locally defined `duration` + `unit` traits. Those can be refactored to compose Duration as a child.
