...
| unit | $EnumeratedToken | yes | The unit of measurement, e.g. `$kg`, `$cm`, `$inches`, `$celsius`. Consuming schemas may constrain the allowed units. |
...
- The `unit` trait uses `$EnumeratedToken` so consuming schemas can constrain accepted units via the "leaves are open, composers constrain" pattern.
