...
| ratingKind | $EnumeratedToken | no | What aspect is being rated, e.g. `$overall`, `$quality`, `$difficulty`, `$taste`, `$value`. Consuming schemas constrain the allowed values. |

## Design Notes

- Only `ratingValue` is required. When `bestRating` and `worstRating` are omitted, the conventional interpretation is a 1-to-5 scale, but consuming schemas define their scale explicitly via documentation or constraints.
- Multiple ratings on different aspects are expressed by composing several Rating children, each with a distinct `ratingKind`.
