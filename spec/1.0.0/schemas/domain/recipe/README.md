...
| Yield | Semantic | ForbidsContent | `measure:Measure` (0..1), `massmeasure:MassMeasure` (0..1), `volumemeasure:VolumeMeasure` (0..1) | The produced quantity when the result is better expressed as output amount rather than serving count. Traits: `amount` (required), `item`. |
...
| Ingredient | Semantic | ForbidsContent | `measure:Measure` (0..1), `massmeasure:MassMeasure` (0..1), `volumemeasure:VolumeMeasure` (0..1) | A single ingredient entry. Traits: `food` (required), `preparation`, `optional`, `toTaste`. |
